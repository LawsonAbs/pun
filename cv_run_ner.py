    ...
    for train_index, test_index in kf.split(all_examples):
        ...
        for index in trange(int(args.num_train_epochs), desc="Train Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Train Iteration")):
                ...
                train_all_sense = getSenseEmbedding(input_ids,args.bert_model)
                defi_emb = []
                all_sense = []
                ...
                # print(all_sense)                                
                inputs = auto_tokenizer(all_sense,return_tensors='pt',padding='max_length',max_length=50)
                
                # bert处理的大小是 batch_size * max_seq_length(75) * defi_num(10) 条句子
                output = sense_bert(**inputs)
                last_layer,other = output
                # last_layer size = [batch_size * max_seq_length(75) * defi_num(10), max_length(50),768 ]
                all_cls_emb = last_layer[:,0,:] # 所有句子的cls 向量
                
                defi_emb = all_cls_emb.view(args.train_batch_size,args.max_seq_length,10,768)
            ...
            for input_ids, input_mask, segment_ids, label_ids, prons_ids, prons_att_mask in tqdm(eval_dataloader, desc="Evaluating Iterator"):
                ...
                all_cls_emb = last_layer[:,0,:] # 所有句子的cls 向量
                # 得到的
                defi_emb = all_cls_emb.view(args.eval_batch_size,args.max_seq_length,10,768)

                if not args.do_pron: prons_emb = None

                with torch.no_grad(): # 不回归计算
                    # 这里判断的原因是是否返回计算出的attention 值
                    if args.do_pron:                         
                        logits,att = model(input_ids, segment_ids, input_mask, prons_emb, prons_att_mask,defi_emb)
                    else:                        
